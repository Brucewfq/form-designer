	...
	processResponse : function(response, node, callback){
		...
		try {
			...
			if(pi.limit < pi.total){
               ...
               //如果分页栏是存在的，但是总数小于允许的最大值，则将隐藏分页工具栏显示出来
                 node.attributes.ptb.setVisible(true);
                }
			}else{
                if(node.attributes.ptb){
                //如果分页栏是存在的，但是总数小于允许的最大值，则隐藏分页工具栏
		...
		if(this.ptb !== ptb){
			...
			if(ptb == undefined){
				...
				if(!showOnTop){
					...
					if(Ext.isIE){
						el = Ext.DomHelper.append(el,{tag:'div',style:'display: inline;white-space:nowrap;'},true);
					}else{
...
Ext.PagingButton = Ext.extend(Ext.Button,{
	...
	onRender : function(ct, position){
		if(!this.template){
			...
			'<tr><td class="x-btn-mc"><em class="{5}" unselectable="on"><button class="x-btn-text {2}" type="{1}">{0}</button></em></td></tr>',
			"</tbody></table>");
