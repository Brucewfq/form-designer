...
*		})
* 
* 	author: Ancuta Florin Vasile
* 		(web developer)
* 
* 	Warning: Still need to test and to fix this component. It's a (alpha). :)
*
* 
* 	created: 24.03.2008
* 	Please report any bug at: achebv(at)yahoo(dot)com[achebv@yahoo.com] or achebv(at)gmail(dot)com[achebv@gmail.com]  			
* */
...
Ext.extend(Ext.ux.MoneyField, Ext.form.NumberField, {
    selectOnFocus: true, // default select value
    //	custom config:
    // numsSpace: space between numbers : 123(numsSpace)456(numsSpace) | for ex: 123 456 
    numsSpace: ",",
    // moneySimbol: simbol for money: like $, €... 
    moneySimbol: "",
    // precision: if you need to use decimal precision. Put allso 'decimalPrecision': (number).
    precision: this.precision !== undefined ? this.precision : false,
    ...
    */
    allowNegative: this.allowNegative !== undefined ? this.allowNegative : true,

    baseChars: this.baseChars !== undefined ? this.baseChars : "0123456789",
    ...
    getValue: function () {
        if (this.hiddenField) {
            if (this.precision) {
                return this.fixPrecision(this.parseValue(this.hiddenField.value));
            } else {
                return this.parseValue(this.hiddenField.value);
            }
        }
        var v = this.fixPrecision(this.parseValue(Ext.form.NumberField.superclass.getValue.call(this)));
        return parseFloat(v);
    },
    ...
    setValue: function (v) {
        ...
        if (this.hiddenField) {
            ...
            this.hiddenValue = this.hiddenField.value;
        }

        this.displayValue = this.moneySimbol + this.moneyFormat(v, this.decimalSeparator, this.numsSpace);
        Ext.form.NumberField.superclass.setValue.call(this, this.displayValue);
    },

    // private
    parseValue: function (value) {

        value = String(value).replace(this.numsSpace, "").replace(this.moneySimbol, "");
        value = this.removeSpaces(value);
        value = parseFloat(String(value).replace(this.decimalSeparator, "."));
        return isNaN(value) ? '' : value.toFixed(this.decimalPrecision);
    },
    ...
    moneyFormat: function (num, decpoint, sep) {
        if (num == 'undefined' || num == "")
            return num;
        num = parseFloat(num).toFixed(this.decimalPrecision)
        var ps = num.split('.');
        var whole = ps[0];
        var sub = ps[1];
        var r = /(\d+)(\d{3})/;
        while (r.test(whole)) {
            whole = whole.replace(r, '$1' + sep + '$2');
        }
        if (this.decimalPrecision <= 0)
            v = whole + sub;
        else
            v = whole + decpoint + sub;
        return v;
    ...
    removeSpaces: function (string) {
        var tstring = "";
        string = '' + string;
        string = String(string).replace(this.moneySimbol, " ");
        splitstring = string.split(this.numsSpace);
    ...
    beforeBlur: function () {
        var v = this.removeSpaces(this.getRawValue());
        v = this.parseValue(v); //	float value... here	
        if (v) {
            if (this.precision) {
    ...
    isValid: function (v) {
        if (!v)
            v = this.getValue();
        return this.validateValue(v);
    },
    //	private
    validateValue: function (value) {
        try {
            if (this.isVisible() == false)//if the hide not valid!
                return true;
    ...
    initEvents: function () {
        ...
        var allowed = this.baseChars + '' + this.numsSpace + this.moneySimbol;
